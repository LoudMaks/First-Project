...
class PlotsBuilder():
    ...
    def date_calculate(self, dataset, plot_type):
        if plot_type == PlotType.PLOT_BY_DAY_SUM:
            return self.calculate_plot_by_sum(dataset)
        elif plot_type == PlotType.PLOT_BY_DAY_CHECK_COUNT:
            return self.calculate_plot_by_check_count(dataset)
        else:
            return []
    
    def calculate_plot_by_sum(self, dataset):
            result = dict()
            i = 0
            while i < len(dataset):
                sum = 0
    ...
    def calculate_plot_by_check_count(self, dataset):
        result = dict()
