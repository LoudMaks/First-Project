...
from plot_builder import PlotsBuilder
from tkcalendar import Calendar
import tkinter as tk
from tkinter import messagebox
from plot_constant import PlotType, PlotViewType
...
class PlotsWindow(BaseSubWindow):
    ...
    def create_plot_type_menu(self):
        ...
        self.plot_type = tk.OptionMenu(self.root, self.selected_plot_type, *self.plot_types)
        self.plot_type.grid(row = 3, column = 0)

    def crete_type_plot_menu(self):
        ...
        self.plot_view_types = tk.OptionMenu(self.root, self.selected_plot_view_type, *self.plot_view_types)
        self.plot_view_types.grid(row = 4, column = 0)
    ...
    def create_ui(self):
        ...
        self.button_make_plot = tk.Button(self.root, text = "Побудувати шрафік", command = self.make_plot)
    ...
    def make_plot(self):
        if self.builder.date_to is not None and self.builder.date_from is not None and self.builder.date_to < self.builder.date_from:
            messagebox.showerror("Помилка", "Дата кінця не може бути раніше дати початку")
            return
        else:
            ...
            if plot_type == "Графік кількості продажів":
                self.builder.add_plot_type(PlotType.PLOT_BY_DAY_CHECK_COUNT).build()
            elif plot_type == "Графік сум продажів":
                self.builder.add_plot_type(PlotType.PLOT_BY_DAY_SUM).build()
